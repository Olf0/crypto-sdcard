...
3. All RPMs for a single version built from the release tarballs must contain mutually exclusive dependecies, otherwise the dependecy resolver on a target system might pick a wrong RPM to install.<br />
   ...
   * Above "dependency pair" forms 2 and 3 can be "chained" to create more than two different releases depending on versions of another RPM (becoming "dependency triples", quadruples etc.), e.g. fot three different version dependent releases:<br />
